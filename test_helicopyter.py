...
def test_only_main() -> None:
    """Only data sources and resources inside gain the count guard."""
    ...
    assert guard in rendered['data.github_user.coving_tron']
...
def test_multisynth_isolates_deploys(tmp_path: Path, monkeypatch: MonkeyPatch) -> None:
    ...
    for cona, body in {
        ...
        (tmp_path / 'deploys' / cona / 'terraform' / 'main.py').write_text(dedent(body).lstrip())
    ...
    for name in [module for module in modules if module.startswith('deploys')]:
        del modules[name]
    assert (tmp_path / 'deploys' / 'aaa' / 'terraform' / 'main.tf').read_text() == dedent("""
    ...
    """).lstrip()
    assert (tmp_path / 'deploys' / 'bbb' / 'terraform' / 'main.tf').read_text() == dedent("""
        # AUTOGENERATED by helicopyter
