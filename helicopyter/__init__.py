        ...
        if hashicorp_configuration_language:
            ...
            try:
                autoformatted = check_output(  # noqa: S603
                    [format_with, *([] if format_with == 'cat' else ['fmt', '-'])],
                ...
                print(
                    f'{format_with} --version:',
                    check_output([format_with, '--version'], stderr=PIPE).decode(),  # noqa: S603
                )
                raise
            formatted = sub(
                r'\n{3,}',
