...
def synth(cona: str, base: Path, relative_output: str) -> None:
    print(f'Generating {relative_output}')
    module = import_module(relative_output.removesuffix('.tf.json').replace('/', '.'))
    ...
    with (base / relative_output).open('w') as opened:
        dump(dictionary, opened, indent=4, sort_keys=True)
        opened.write('\n')
...
def multisynth(all_or_conas_or_paths: Iterable[str]) -> None:
    ...
    for cona_or_path in conas_or_paths:
        ...
        if (
            ...
            cona = cona_or_path
        synth(cona, Path(__file__).parent, f'deploys/{cona}/terraform/main.tf.json')
